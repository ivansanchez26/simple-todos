...
const SongImages = new FilesCollection({
  ...
  onBeforeUpload: function (file) {
    ...
    if (file.size <= 1024 * 1024 * 5 && /png|jpe?g/i.test(file.extension)) { //&& /png|jpe?g/i.test(file.extension)
      return true;
    }
    return 'Please upload images in png/jpg/jpeg extension, with size equal or less than 5MB';
  }
});
...
Meteor.methods({
  'songImage.remove'(songImageId) {
    SongImages.remove({_id: songImageId});
  }
});
